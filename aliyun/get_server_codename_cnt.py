...
data_dir = '/xxxxxxxxxxxxxxxx'
#data_dir = ""
def get_access():
    conf = configparser.ConfigParser()
    conf.read("xxxxxxxxxxxxxx", encoding="utf8")
    username = conf.get('access', 'username')
    password = conf.get('access', 'password')
...
def get_Project_Department_Codename():  # 调接口获取项目/部门/Item对应关系
    ...
    while i <= 3:
        ...
        for j in range(len(res1[u'results'])):
            res.append(res1[u'results'][j])
        i = i+1
    res_data[u'results'] = res
...
def zabbix_sender_date():
    ...
    cmd_zabix_sender_data = 'cat '+data_dir+'codename_count_data.json | grep ecs | sed -e \'s/"//g\'' \
                            ' -e \'s/:/ /g\' -e \'s/,//\' -e \'s/^/"k8s-node3"/\'' \
                            ' -e \'s/_/_[/\' -e \'s/_[B-Z]/&]/\' -e \'s/All/&]/\' > zabbix_sendfile_codename'

    commands.getoutput(cmd_zabix_sender_data)
    time.sleep(2)
    zabbix_file = 'zabbix_sender -z 192.168.81.100 -i zabbix_sendfile_codename > /dev/null 2>&1'
